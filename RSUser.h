...
class RSUser
{
 private:
  ...
  std::shared_ptr<RecommenderSystem> _recommender_sys;

public:
	/**
	 * Constructor for the class
	 */
	 RSUser(std::string name, rank_map movies_rank,
            std::shared_ptr<RecommenderSystem>
     recommender_sys)
     :_user_name(name),_movies_rank(movies_rank) ,
     _recommender_sys(std::move(recommender_sys))
     {}
